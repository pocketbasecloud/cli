...
1. **Deploy once from your own computer** (`pb cloud login`, then
   `pb cloud frontend deploy` — or `pb cloud pb deploy` /
   `pb cloud backend deploy`). It asks which project and what to call this, and
   writes the answers to `pb.json`. **Commit that file** — it is what makes the
...
2. **Add the token.** Portal → **Account** → **CLI access token** → **Copy**,
   ...
   repository secret**, named `PB_TOKEN`.

`pb cloud ci init` writes the workflow file for you, filled in from the
directory's `pb.json`, and prints those two steps with your project's details
in them.
...
| `kind` | `auto` | `auto`, `pb`, `frontend`, or `backend`. `auto` lets the CLI decide from `pb.json` and the directory. |
| `working-directory` | `.` | The directory holding `pb.json`. |
| `cli-version` | the action's own version | Which `@pocketbasecloud/cli` to install. |
| `project` | — | Project name or id. Only needed when `pb.json` is not committed. |
| `name` | — | Resource name. Only needed when `pb.json` does not name one. |
| `env` | — | The `pb.json` environment to deploy. |
| `compute` | — | Compute id, for Pro accounts with more than one. |
| `location` | — | Region code for a first deploy. `pb cloud locations` lists them. |
...
- **Admin credentials never reach the log.** A PocketBase deploy's record
  ...
  is the reason to use the action rather than call `pb ... --json` yourself.
...
- **Frontend environment variables are baked in at build time**, so they belong
  ...
  platform — set them once with `pb cloud env set`.
- Full guides:
