...
export const DEPLOY_USAGE =
  "Usage: pb cloud deploy [pb|frontend|backend] [<name>]";
 ...
 * way. `pb cloud init` is offered too, because recording the answer once is
...
/**
 * `pb cloud deploy` — the deploy command you can run without first deciding
 ...
 * works on `pb cloud frontend deploy` works here and there is exactly one
...
export function makeDeployCommands(
  ...
  const deploy: Handler = async (ctx: CmdCtx) => {
    const cwd = deps.cwd();
    // A leading kind word is an override, not a resource name: `pb cloud
...
function report(guess: KindGuess, ctx: CmdCtx): void {
  ...
  console.log(
    `Detected a ${kindDisplay(guess.kind)} (${guess.reason}) — running ` +
      `\`pb cloud ${kindCommand(guess.kind)} deploy\`.`,
  );
}
