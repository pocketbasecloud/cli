 ...
 * answers the question before it — the one `pb cloud deploy` exists to stop
 ...
 * below, and `pb cloud deploy frontend` settles any case those get wrong.
 */
 ...
 * detects the same kind its deploy would target. A pb.json with a `kind` but
...
async function boundKind(cwd: string): Promise<KindGuess | null> {
  const linked = (await readLinkFile(cwd))?.kind ??
    (await readOwnPbJson(cwd)).kind;
  return linked
    ? { kind: linked, reason: "pb.json binds this directory" }
    : null;
}

/**
 * What `pb cloud deploy` should deploy, or null when the directory says
 * nothing either way.
 *
